...
tar_info <- readRDS("../data/info-inmet-sul.rds") %>% arrange(site)
tar_info
```
...
t_data <- readRDS("../data/data-raw-inmet-sul-20000922-20161231.rds") %>%
  select(site:tair, tmin, tmax, td, tdmax, tdmin, prec, p, rg) %>% 
  arrange(site)

n <- which(names(t_data) == 'tair')
names(t_data) [n] <- paste0('tinst')

t_data

# glimpse(data)
#"A825" %in% unique(data$site)
```

# Adicionando coluna para "tmean"

```{r}
tar_data <- t_data %>% 
  mutate(tavg = (tmax + tmin) / 2) %>% 
  select(site, date, tinst, tavg, everything())
tar_data
```
...
```{r save1}
tar_data_p <- tar_data_pad %>% 
  arrange(site) %>% 
  select(site, everything())
tar_data_p

saveRDS(tar_data_p, "../data_saves/tar_data_pad.rds")

rm(tar_data_pad, tar_data_p, t_data, tar_data)
```

```{r load1, warning=TRUE}
tar_data_pad <- readRDS("../data_saves/tar_data_pad.rds")
tar_data_pad
```
...
```{#r}
 tar_data_s <- tar_data_sel %>% 
   select(c('site', 'date', 'tair', 'tmean', 'tmin', 'tmax', 'prec', 'p', 'rg')) %>% 
   arrange(site)
# tar_data_s

# saveRDS(tar_data_s, "../outdata/tar_data_sel.rds")
tar_data_sel <- readRDS("../outdata/tar_data_sel.rds")
tar_data_sel
```
...
saveRDS(tar_data_s, "../data_saves/tar_data_sel.rds")
```

```{r load2, warning=TRUE}
rm(tar_data_sel, tar_data_s, tar_data_pad, tar_data_pad_inf)

tar_data_sel <- readRDS("../data_saves/tar_data_sel.rds")
tar_data_sel
```
...
file_info_sel <- paste0("tar-info-inmet-", 
                        ...
                        period_thres, "yrs-",
                        "south.rds")
tar_info_sel <- filter(tar_info, site %in% sites_sel) %>% arrange(site)
tar_info_sel
...
tar_gaps <- tar_data_sel %>%
  ...
  dplyr::summarise(# % de dados faltantes
                   missing = round(percent_NA(tinst), 1),
                   valid = 100 - missing,
                   #long_valid = length(na.contiguous(tair)),
                   long_valid = longest_gap(tinst, reverse = TRUE),
                   # tamnho da falha mais longa
                   long_gap = longest_gap(tinst),
                   #data de início da maior falha
                   date_slg = date_longest_gap(tinst, date),
                   date_slv = date_longest_gap(tinst, date, reverse = TRUE)
                   )  %>%
  # desagrupando por site
  ungroup() %>%
...
sites_ord <- records_month %>%
  group_by(site) %>%
  summarise(disp = mean(tinst, na.rm = TRUE)) %>%
  arrange(desc(disp)) %>%
  #select(site) %>%
...
records_month_plot <- records_month %>%
  mutate(tinst = ifelse(tinst > 0, tinst, NA),
...
ggp <- ggplot(aes(x = date,
  ...
  geom_point(aes(colour = tinst), shape=15, size = 2.6) + 
  labs(x = "Ano", y = "EMA") +
