---
title: "Controle de qualidade 1a - Teste dos Limites do Intervalo de Variação"
subtitle: "Baseado em limites estabelecidos pelo intervalo de medida instrumental"
...
```{r}
# Definições iniciais
ID <- '1a' # identificador do teste de controle de qualidade aplicado
...
Neste caso a variável usada como rótulo identificador de dado suspeito será composto pela variável `tavg` (temperatura média do ar) e o identificador correspondente ao **teste dos limites do intervalo de variação** (`qc1a`). A nova variável adicionada ao conjunto de dados original será nomeada `tavg_qc1a` que assumirá os valores

  - `tavg_qc1a = 1`, dado não aprovado no teste `qc1a`, portanto é classificado como uma observação de `tavg` **suspeita**
  
  - `tavg_qc1a = 0`, dado aprovado no teste `qc1a`
...
filter(var1_data_qc, tavg_qc1a == 1) %>% arrange(site)
```
...
## var1_nr_susp <- var1_data_qc %>% filter(tavg_qc1a == 1) %>% nrow()
...
Foram adicionadas 3 colunas a tabela de metadados do `qc1a`: `tmin`, `tmed`, `tmax` para verificar-se essas estatísticas durante o evento.
...
var1_summary_qc <-
  ...
  summarise(
    qc = first(qc),
    tot = sum(duration), # tot: total de dados suspeitos por site (por horas)
    n_events = max(id),
    n_max = max(duration)) %>%
  ungroup() %>%
