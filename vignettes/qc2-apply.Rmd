---
title: "Controle de qualidade 2 - Teste de Persistência Temporal"
subtitle: "Baseado em dados repetidos"
...
```{r}
# Definições iniciais
ID <- '2' # identificador do teste de controle de qualidade aplicado
...
Neste caso a variável usada como rótulo identificador de dado suspeito será composto pelas variável `tavg` (temperatura média do ar) e o identificador correspondente ao **teste de persistência temporal** (`qc2`). A nova variável adicionada ao conjunto de dados original será nomeada `tavg_qc2` que assumirá o valor

  - `tavg_qc2 = 1`, dado não aprovado no teste `qc2`, portanto é classificado como uma observação de `tavg` **suspeita**
  
  - `tavg_qc2 = 0`, dado aprovado no teste `qc2`
...
filter(var1_data_qc, tavg_qc2 == 1) %>% arrange(site)
```
...
## var1_nr_susp <- var1_data_qc %>% filter(tavg_qc2 == 1) %>% nrow()
...
Neste caso, como `value = tmin = tmed = tmax`, não serão colocadas essas varíaveis para evitar redundância.
