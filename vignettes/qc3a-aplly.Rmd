---
title: "Controle de qualidade 3a - Teste de Consistência Interna"
subtitle: "Baseado na comparação $T_{min}(h) ≥ T_{max}(h)$"
...
```{r}
# Definições iniciais
ID <- '3a' # identificador do teste de controle de qualidade aplicado
ID_title <- paste0('QC_',ID) # identificador usado nos mapas e gráficos
var1 <- 'tmin' # variável 1
var2 <- 'tmax' # variável 2
...
Neste caso a variáveL usada como rótulo identificador de dado suspeito será composto pelaa variável `tmin` (temperatura mínima do ar) e o identificador correspondente ao **teste de consistência interna** (`qc3a`). A nova variável adicionada ao conjunto de dados original será nomeada `tmin_qc3a` que assumirá os valores

  - `tmin_qc3a = 1`, dado não aprovado no teste `qc3a`, portanto é classificado como uma observação de `tmin` **suspeita**
  
  - `tmin_qc3a = 0`, dado aprovado no teste `qc3a`
...
filter(var1_data_qc, tmin_qc3a == 1) %>% arrange(site)
```
...
var1_meta_qc <-
  ...
  dplyr::summarise(
    ...
    tmin = fun_NA(tmin, min, na.rm = TRUE),
    tmed = fun_NA(tmin, mean, na.rm = TRUE),
    tmax = fun_NA(tmin, max, na.rm = TRUE)) %>%
  ungroup() %>%
  # reordenar
  select(site, qc:end, id) %>%
  arrange(site, desc(duration))
var1_meta_qc

# Total de casos suspeitos:
## var1_nr_susp <- var1_data_qc %>% filter(tmin_qc3a == 1) %>% nrow()
