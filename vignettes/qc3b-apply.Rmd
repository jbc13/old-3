---
title: "Controle de qualidade 3b - Teste de Consistência Interna"
subtitle: "Baseado na comparação $T_{inst}(h) < T_{min}(h)$ ou $T_{inst}(h) > T_{max}(h)$"
...
```{r}
# Definições iniciais
ID <- '3b' # identificador do teste de controle de qualidade aplicado
ID_title <- paste0('QC_',ID) # identificador usado nos mapas e gráficos
var1 <- 'tinst' # variável 1
...
Neste caso a variável usada como rótulo identificador de dado suspeito será composto pela variável `tinst` (temperatura instantânea do ar) e o identificador correspondente ao **teste de consistência interna** (`qc3b`). A nova variável adicionada ao conjunto de dados original será nomeada `tinst_qc3b` que assumirá os valores

  - `tinst_qc3b = 1`, dado não aprovado no teste `qc3b`, portanto é classificado como uma observação de `tinst` **suspeita**
  
  - `tinst_qc3b = 0`, dado aprovado no teste `qc3b`
...
filter(var1_data_qc, tinst_qc3b == 1) %>% arrange(site)
```
...
var1_meta_qc <-
  ...
  dplyr::summarise(
    ...
    tmin = fun_NA(tinst, min, na.rm = TRUE),
    tmed = fun_NA(tinst, mean, na.rm = TRUE),
    tmax = fun_NA(tinst, max, na.rm = TRUE)) %>%
  ungroup() %>%
  # reordenar
...
## var1_nr_susp <- var1_data_qc %>% filter(tinst_qc3b == 1) %>% nrow()
