---
title: "Controle de qualidade 3c - Teste de Consistência Interna"
subtitle: "Baseado na comparação $T_{avg}(h) < T_{d\_avg\_f}(h)$"
...
```{r}
# Definições iniciais
ID <- '3c' # identificador do teste de controle de qualidade aplicado
ID_title <- paste0('QC_',ID) # identificador usado nos mapas e gráficos
var1 <- 'tavg' # Variável 1
var2 <- 'tdavg_filter' # Variável 2
...
```{r}
# Local onde os dados filtrados estão
td_file <- '../data/data_tdavg_filter.rds'

# Importação dos dados filtrados
td_data <- readRDS(td_file) %>%
  arrange(site) %>%
  select(tdavg_filter, tdmin_qc3a, tdinst_qc3b)
td_data

rm(td_file)

# Unindo dados filtrados com os dados originais
var_data <- data.table::data.table(var_data, td_data)
var_data
...
Neste caso a variável usada como rótulo identificador de dado suspeito será composto pela variável `tavg` (temperatura média do ar) e o identificador correspondente ao **teste de consistência interna** (`qc3c`). A nova variável adicionada ao conjunto de dados original será nomeada `tavg_qc3c` que assumirá os valores

  - `tavg_qc3c = 1`, dado não aprovado no teste `qc3c`, portanto é classificado como uma observação de `tavg` **suspeita**
  
  - `tavg_qc3c = 0`, dado aprovado no teste `qc3c`
...
filter(var1_data_qc, tavg_qc3c == 1) %>% arrange(site)
```
...
## var1_nr_susp <- var1_data_qc %>% filter(tavg_qc3c == 1) %>% nrow()
