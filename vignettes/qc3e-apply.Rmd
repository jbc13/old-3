---
title: "Controle de qualidade 3e - Teste de Consistência Interna"
subtitle: "Baseado na comparação $T_{max}(d) < T_{min}(d-1)$"
...
```{r}
# Definições iniciais
ID <- '3e' # identificador do teste de controle de qualidade aplicado
ID_title <- paste0('QC_',ID) # identificador usado nos mapas e gráficos
var1 <- 'tmax' # Variável 1
...
Neste caso a variável usada como rótulo identificador de dado suspeito será composto pela variável `tmax` (temperatura máxima do ar) e o identificador correspondente ao **teste de consistência interna** (`qc3e`). A nova variável adicionada ao conjunto de dados original será nomeada `tmax_qc3e` que assumirá os valores

  - `tmax_qc3e = 1`, dado não aprovado no teste `qc3e`, portanto é classificado como uma observação de `tmax` **suspeita**
  
  - `tmax_qc3e = 0`, dado aprovado no teste `qc3e`
...
filter(var1_data_qc, tmax_qc3e == 1) %>% arrange(site)
```
...
var1_meta_qc <-
  ...
  dplyr::summarise(
    ...
    tmin = fun_NA(tmax, min, na.rm = TRUE),
    tmed = fun_NA(tmax, mean, na.rm = TRUE),
    tmax = fun_NA(tmax, max, na.rm = TRUE)) %>%
  ungroup() %>%
  # reordenar
...
## var1_nr_susp <- var1_data_qc %>% filter(tmax_qc3e == 1) %>% nrow()
