---
title: "Controle de qualidade 3f - Teste de Consistência Interna"
subtitle: "Baseado na comparação $T_{min}(d) ≥ T_{max}(d-1)$"
...
```{r}
# Definições iniciais
ID <- '3f' # identificador do teste de controle de qualidade aplicado
...
Neste caso a variável usada como rótulo identificador de dado suspeito será composto pela variável `tmin` (temperatura máxima do ar) e o identificador correspondente ao **teste de consistência interna** (`qc3f`). A nova variável adicionada ao conjunto de dados original será nomeada `tmin_qc3f` que assumirá os valores

  - `tmin_qc3f = 1`, dado não aprovado no teste `qc3f`, portanto é classificado como uma observação de `tmin` **suspeita**
  
  - `tmin_qc3f = 0`, dado aprovado no teste `qc3f`
...
filter(var1_data_qc, tmin_qc3f == 1) %>% arrange(site)
```
...
## var1_nr_susp <- var1_data_qc %>% filter(tmin_qc3f == 1) %>% nrow()
